...
class_courses = Table(
    ...
    Column('course_id', UUID(as_uuid=True), ForeignKey('courses.id'), nullable=False, index=True),
...
class Class(Base):
    ...
    course_id = Column(UUID(as_uuid=True), ForeignKey("courses.id"), nullable=True, index=True)  # 改为可空，保持向后兼容
    teacher_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True)
