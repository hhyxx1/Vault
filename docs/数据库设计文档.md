...
| student_id | VARCHAR | 50 | 否 | - | 学号（学生专用） |
| employee_id | VARCHAR | 50 | 否 | - | 工号（教师专用） |
| major | VARCHAR | 100 | 否 | - | 专业（学生） |
| department | VARCHAR | 100 | 否 | - | 部门（教师） |
| grade | VARCHAR | 20 | 否 | - | 年级（学生） |
| class_name | VARCHAR | 50 | 否 | - | 班级（学生） |
...
| status | VARCHAR | 20 | 是 | 'draft' | 状态：draft/active/closed/archived |
...
| question_type | VARCHAR | 50 | 是 | - | 题型：single_choice/multiple_choice/true_false/fill_blank/essay |
| question_text | TEXT | - | 是 | - | 题目内容 |
| question_order | INTEGER | - | 是 | - | 题目顺序 |
| score | INTEGER | - | 是 | 0 | 题目分值 |
