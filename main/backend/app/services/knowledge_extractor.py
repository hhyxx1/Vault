...
class KnowledgePointExtractor:
    ...
    async def process_document_async(
        ...
        try:
            ...
            # 步骤2：分割章节 (20-30%)
            sections = self._split_comprehensive(text_content)
    ...
    def _split_comprehensive(self, text_content: str) -> List[Dict[str, Any]]:
        """全面分割文本 - 识别所有可能的章节结构"""
        sections = []
        ...
        for pattern, title_group in patterns:
            matches = list(re.finditer(pattern, text_content, re.MULTILINE | re.DOTALL | re.IGNORECASE))
            if len(matches) >= 1:
        ...
        # 按双换行分段
        paragraphs = [p.strip() for p in re.split(r'\n\s*\n', text_content) if len(p.strip()) > 50]
        if paragraphs:
    ...
    def _extract_all_subsections(self, text_content: str) -> List[Dict[str, str]]:
        """提取所有小节 - 无限制"""
        subsections = []
        patterns = [
            r'(\d+\.\d+[\s\u3000：:]+)(.+?)(?=\d+\.\d+[\s\u3000：:]|$)',
        ...
        for pattern in patterns:
            matches = list(re.finditer(pattern, text_content, re.MULTILINE | re.DOTALL))
            if len(matches) >= 2:
    ...
    def _extract_all_definitions(self, text_content: str) -> List[Dict[str, str]]:
        """提取所有定义 - 无限制"""
        definitions = []
        patterns = [
            r'(.{2,80})是指(.{5,500})(?=[。\n])',
    ...
    def _extract_list_items(self, text_content: str) -> List[Dict[str, str]]:
        """提取列表项"""
        items = []
        patterns = [
            r'^[\s]*[•·\-*][\s]+(.+?)$',
    ...
    def _extract_examples(self, text_content: str) -> List[Dict[str, str]]:
        """提取示例"""
        examples = []
        patterns = [
            r'[【\[]?例[\d]*[】\]]?[:：](.+?)(?=[【\[]?例[\d]*[】\]]?|$)',
    ...
    def _extract_keywords_advanced(self, text_content: str) -> List[str]:
        """高级关键词提取"""
        # 中文分词
        words = re.findall(r'[\u4e00-\u9fa5]{2,}', text_content)
        # 英文词
        en_words = re.findall(r'[a-zA-Z]{3,}', text_content)
