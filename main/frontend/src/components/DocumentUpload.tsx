...
import { getDocumentProgress, uploadDocument } from '../services/documents';
...
const DocumentUpload: React.FC<DocumentUploadProps> = ({ courseId, onUploadComplete }) => {
    ...
    const interval = setInterval(async () => {
      for (const upload of processingUploads) {
        try {
          ...
          const data = await getDocumentProgress(upload.documentId);
  ...
  const uploadFile = async (file: File, documentType: 'outline' | 'material') => {
    try {
      ...
      setUploads(prev => prev.map(u => 
        u.file === file
          ? { ...u, status: 'error', error: error.message || String(error) }
          : u
      ));
    }
  };
