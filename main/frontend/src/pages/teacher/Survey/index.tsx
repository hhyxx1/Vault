...
import { getTeacherClasses, getTeacherCourses } from '@/services/teacher'
  ...
  const handleGenerate = async () => {
    if (createMode === 'ai' || createMode === 'knowledge') {
      ...
      try {
        ...
        // 使用 surveyGenerationApi 替代直接的 axios 调用
        const response: any = createMode === 'ai' 
          ? await surveyGenerationApi.generateAI(payload)
          : await surveyGenerationApi.generateKnowledgeBased(payload)
  ...
  const handleSaveQuestions = async () => {
    ...
    try {
      ...
      if (aiGeneratedData) {
        ...
        // 调用保存API
        const result: any = await surveyGenerationApi.saveSurvey({
