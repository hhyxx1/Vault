...
项目中的向量数据库路径**不再写死**，由配置统一管理：

- **配置文件**：`backend/app/config/settings.py` 中的 `VECTOR_DB_PATH`（默认 `"./data/chroma_db"`）。
- **环境变量**：可通过环境变量覆盖，例如 `VECTOR_DB_PATH=D:/my_data/chroma_db` 或 `.env` 中配置。
- 相对路径会相对于 backend 目录解析，绝对路径则直接使用。
